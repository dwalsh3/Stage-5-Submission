...
# These tests are not exhaustive, but thdey should cover the majority of cases.
#
...
def testPairings():
    ...
    pairings = swissPairings(standings)
    if len(pairings) != 4:
        raise ValueError(
    ...
    reportMatch(id7, id8)
    # pairings = swissPairings(standings)
    pairings   = swissPairings(playerStandings()) # I changed this to update standings each time pairings is called rather than relying on the last locally stored standings list
    if len(pairings) != 4:
        raise ValueError(
            "For eight players, swissPairings should return 4 pairs. Got {pairs}".format(pairs=len(pairings)))
